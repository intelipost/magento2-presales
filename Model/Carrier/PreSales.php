...
public function collectRates(RateRequest $request, $pickup = false)
{
    ...
    if (is_array ($preSalesItems))
    {
        ...
        foreach ($preSalesItems as $id => $children)
        {
            ...
            if (!empty ($moreDays)) $timestamp = strtotime ($moreDays, $timestamp); // additional days

            $final_timestamp = $timestamp > $min_date ? $timestamp : $min_date;

            $objectRequest->setAdditionalInformation(array('shipped_date' => date('Y-m-d', $final_timestamp)));
    ...
    if (is_array ($preSalesResult))
    {
        ...
        {
            if ($item instanceof \Magento\Quote\Model\Quote\Address\RateResult\Error) return $item;

            $rates = $item->getAllRates ();
            if (is_array ($rates)) $result = array_merge ($result, $rates);
        }
    }
    ...
    if (is_array ($packageItems))
    {
        foreach ($packageItems as $id => $children)
        {
            ...
            $objectRequest->setAdditionalInformation(array('shipped_date' => date('Y-m-d', $timestamp)));
...
*/
    ...
    foreach ($collection as $child)
    {
        ...
        foreach ($result as $item)
        {
            ...
            if (in_array ($itemType, $stored)) continue;
